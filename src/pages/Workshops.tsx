...
import WorkshopPaymentForm from "@/components/WorkshopPaymentForm";
...
const Workshops = () => {
  ...
  const [showWorkshopPaymentForm, setShowWorkshopPaymentForm] = useState(false);
  ...
  const handleWorkshopRegistration = (workshop: Workshop) => {
    setSelectedWorkshopDetails({
      ...
      paymentButtonId: 'pl_R5IOS2nRVp5Gvu', // Default workshop payment button
      date: workshop.date,
      time: workshop.time,
      features: workshop.features.map(feature => ({ text: feature }))
    });
    setShowWorkshopPaymentForm(true);
  };

  const closeWorkshopPaymentForm = () => {
    setShowWorkshopPaymentForm(false);
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#e9f5e9' }}>
      ...
      {/* Workshop Payment Form Modal */}
      {showWorkshopPaymentForm && selectedWorkshopDetails && (
        <WorkshopPaymentForm
          isOpen={showWorkshopPaymentForm}
          onClose={closeWorkshopPaymentForm}
          workshopDetails={selectedWorkshopDetails}
        />
      )}
    </div>
  );
